...
import org.jpos.iso.filter.DebugFilter;
...
public class Main {
    ...
    private static void startMCChannel() {
        NACChannel c = new NACChannel();
        GenericPackager p;
        try {
            p = new GenericPackager("jar:packager/mc_dms.xml");
            c.setPackager(p);
            c.setLogger(Logger.getLogger("Q2"), "mc-channel");
            List<ISOFilter> filters = new ArrayList<>();
            filters.add(new DebugFilter());
            c.setHost("127.0.0.1", 3115);
            c.setIncomingFilters(filters);
            c.connect();
            ISOMsg m = c.receive();
